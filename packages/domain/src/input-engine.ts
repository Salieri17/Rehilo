...
export function parseStructuredCommand(rawInput: string): ParseStructuredCommandResult {
  ...
  return {
    ...
    pathSegments: [title, ...argsRaw.map((arg) => arg.trim()).filter(Boolean)]
  };
}
...
export function structuredCommandToNodeInput(
  ...
  const hasHierarchyPath = pathSegments.length > 1;

  if (hasHierarchyPath) {
    const parentSegments = pathSegments.slice(0, -1);
    const leafTitle = pathSegments[pathSegments.length - 1] ?? parsed.title;
    ...
    return {
      primary: {
        ...
        title: leafTitle,
        content: "",
        ...(parsed.nodeType === "todo" ? { status: "pending" as const } : {}),
        metadata: {
          hierarchyPath: pathSegments,
          hierarchyMode: "path-command",
          hierarchyDepth: pathSegments.length - 1
        }
      },
      secondary: parentSegments.map((segment, index) => ({
        workspaceId: options.workspaceId,
        type: "project",
        title: segment,
        content: "",
        metadata: {
          hierarchyPath: pathSegments.slice(0, index + 1),
          autoGeneratedParent: true,
          hierarchyMode: "path-command"
        }
      }))
    };
